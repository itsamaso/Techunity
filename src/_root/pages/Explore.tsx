...
const SearchResults = ({ isSearchFetching, searchedPosts }: SearchResultProps) => {
  if (isSearchFetching) {
    ...
    return <GridPostList posts={searchedPosts.documents} showSimplifiedUI={true} />;
  } else {
    return (
...
const Explore = () => {
  ...
  const shouldShowNoPosts = !shouldShowSearchResults && 
    posts.pages.every((item) => item.documents.length === 0);

  // Only flatten posts when not showing search results, and deduplicate by post ID
  const allPosts = !shouldShowSearchResults 
    ? posts.pages
  ...
  return (
    <div className="flex flex-1">
      <div className="common-container">
        <PageHeader
          title="Explore"
          subtitle="Discover trending posts and search through our community"
          icon={
        ...
        <div className="flex flex-wrap gap-9 w-full max-w-5xl">
          {shouldShowSearchResults ? (
            ...
            <GridPostList posts={allPosts} showSimplifiedUI={true} />
          )}
        </div>
